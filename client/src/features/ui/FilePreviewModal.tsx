...
export function FilePreviewModal({ file, isOpen, onClose }: FilePreviewModalProps) {
  ...
  const getFileTypeLabel = (fileName: string) => {
    ...
    switch (ext) {
      ...
      case 'doc':
      case 'docx': return 'Word Sənəd';
      case 'xls':
      case 'xlsx': return 'Excel Cədvəl';
      default: return 'Fayl';
    }
  };
  ...
  const getFileTypeColor = (extension: string) => {
    switch (extension) {
      ...
      case 'doc':
      case 'docx': return '#4285f4';
      case 'xls':
      case 'xlsx': return '#34a853';
      ...
      title={
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-lg bg-[#e8f0fe] flex items-center justify-center">
            <svg className="w-5 h-5" viewBox="0 0 24 24" fill={getFileTypeColor(fileExtension)}>
              <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-6-6zm-1 2l5 5h-5V4z" />
            </svg>
          </div>
          <div>
            <div className="text-base font-semibold text-gray-900 truncate max-w-[400px]">
              {file?.name || 'Fayl önizləməsi'}
            </div>
            <div className="text-xs text-gray-500">
              {file && `${formatFileSize(file.size)} • ${getFileTypeLabel(file.name)}`}
            </div>
          </div>
        </div>
      }
