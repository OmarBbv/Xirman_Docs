...
export default function Layout() {
  ...
  const menuItems = [
    { name: "Ana Səhifə", path: "/dashboard", icon: <HomeIcon /> },
    { name: "Sənədlər", path: "/dashboard/docs", icon: <PostsIcon /> },
    { name: "İstifadəçilər", path: "/dashboard/users", icon: <UsersIcon /> },
    { name: "Ayarlar", path: "/dashboard/settings", icon: <SettingsIcon /> },
  ];
    ...
    <div className="flex h-screen bg-[#f0f0f1] font-sans text-[#1d2327]">
      ...
      >
        ...
        >
          ...
          {!isCollapsed && <span className="text-[12px] ml-1">Menünü yığ</span>}
        </button>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-[32px] bg-[#1d2327] text-[#c3c4c7] flex items-center justify-between px-4 z-10">
          <div className="flex items-center space-x-4 h-full">
            ...
            <div className="hidden md:flex hover:bg-white/10 h-full px-3 items-center cursor-pointer transition-colors space-x-2">
              ...
              <span className="text-[13px]">0 Bildiriş</span>
            </div>
          </div>
          <div className="hover:bg-white/10 h-full px-3 flex items-center cursor-pointer transition-colors space-x-2">
            <span className="text-[13px]">Salam, <span className="font-bold">admin</span></span>
        ...
        <main className="flex-1 overflow-y-auto p-5 max-w-[1500px]">
          <header className="mb-6">
            <h1 className="text-[23px] font-normal text-[#1d2327]">İdarə Paneli</h1>
          </header>
